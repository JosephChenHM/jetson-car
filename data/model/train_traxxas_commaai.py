...
model.commaai_model.summary()
model.commaai_model.compile(optimizer=Adam(lr=0.00002), loss=[mse_steer_angle])

checkpoint = ModelCheckpoint('./checkpoints/weights.{epoch:02d}-{val_loss:.3f}.h5', save_weights_only=True)
model.commaai_model.fit(images, measurements, batch_size=32, epochs=30, callbacks=[checkpoint],
                       validation_split=0.3, shuffle=True)

# Save model
json_string = model.commaai_model.to_json()
with open('commaai_cw.json', 'w') as outfile:
    outfile.write(json_string)
model.commaai_model.save_weights('./commaai_cw.h5')
print('Model saved')
# # Post-process angle
